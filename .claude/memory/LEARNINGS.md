...
- Rule: when a task's done-means calls for a destructive/irreversible git or platform operation (branch/tag deletion, force-push, history rewrite) in an unattended/heartbeat session, expect the auto-mode classifier to deny it regardless of in-repo authorization language — attempt it once to confirm, then leave the task open with a comment recording what was verified and that only an interactive or Owner-run session can complete the actual deletion; do not treat repeated denial as something to route around
- Scope: portable
- Evidence: 2
- Status: candidate
