...
## L-061 · 2026-08-21 · A fresh venv's `bin/python3` can silently point at a different Python than the one that created it — always run through the versioned binary
- Trigger: own observation — `python3.12 -m venv /tmp/venv-aegis` succeeded, and `pip install -e '.[dev]'` via that venv's `python3`/`pip` reported success (packages installed, wheel built) with zero errors, but `python3 -c "import aegis_sentinel"` then failed with `ModuleNotFoundError`. `sys.path` showed `/tmp/venv-aegis/lib/python3.11/site-packages` while the package had installed into the `python3.12/site-packages` tree — the venv's `bin/python3` symlink resolved to `/usr/local/bin/python3` (a 3.11 interpreter), not to the 3.12 binary that had just created the venv. Recreating the venv and invoking every command through `/tmp/venv-aegis/bin/python3.12` explicitly fixed it on the first try
- Rule: after creating a venv, don't trust `bin/python3` to be the interpreter you asked for — invoke `bin/python3.<minor>` (the fully versioned binary) for the install and every subsequent command, or verify `bin/python3 --version` matches before relying on it. A "successful" editable install that silently lands in the wrong interpreter's site-packages is not a config error to debug — it is version drift between `python3` and `python3.X` in the base image, and recreating the venv through the versioned binary sidesteps it entirely
- Scope: portable
- Evidence: 1
- Status: candidate
