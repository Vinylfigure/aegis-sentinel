...
- observed: 2026-08-22 — fired at PR #60 (issue #58): dispatched an adversarial `verifier` agent whose final report said "VERDICT: PASS" with idempotency/vacuous-test/mutation probes as evidence; rather than take the verdict line itself as the checkpoint, `./scripts/verify.sh full` was independently re-run in the main thread after the report landed, and the record count in `artifacts/demo-engagement/verdicts.json` was re-checked by hand before committing — because the report also described deliberately corrupting and restoring state mid-probe, the only way to know the working tree ended in the right place was to re-check it directly, not read the agent's own account of having restored it
...
- observed: 2026-08-22 — FIRED (broke the streak) on PR #60 (issue #58): before writing any code, named the two failure modes explicitly — an ordering dependency if `main_poisons()` merged by reading verdicts.json off disk (only correct if `main()` had already run first), and a second-source-of-truth risk if the poison records got recomputed independently for the new artifact instead of reusing the same objects `poisons.json` already serializes — and designed the merge specifically to make both impossible (`build_poison_artifacts()` calls `build()` itself rather than reading the file; the ten poison records are the literal same dict objects, never recomputed). The dispatched verifier agent's probes (three-run idempotency diff, corrupt-and-detect on the no-second-source-of-truth test) then confirmed both predictions rather than discovering the risks for the first time — the adversarial pass happened before implementation, not only after it
- Scope: portable
- Evidence: 1
