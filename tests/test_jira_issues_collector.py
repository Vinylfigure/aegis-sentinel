"""Jira issues collector acceptance: deterministic collection, per-page
raw hashing, cursor-chain exhaustion recorded page by page, refusal on
a broken or cyclic chain (seeded fixture detected), wrong-account-token
refusal, a page's own claimed issue count exceeding its actual rows
(truncation), null-reporter rows surfaced not dropped, and an EQC with
...
from aegis_sentinel.collectors.jira_issues import (
    canonical_contract_hash,
    collect_jira_issues,
)
from aegis_sentinel.schema import TimeWindow

FIXTURES = Path(__file__).resolve().parent / "fixtures" / "collectors" / "jira_issues"
...
def collect(transport=None, account_token=ACCOUNT_TOKEN, period=PERIOD):
    return collect_jira_issues(
        transport or transport_for(TENANT_DIR),
        account_token=account_token,
        period=period,
        population_ref="pop-jira-issues",
    )
...
def test_repeated_cursor_is_detected():
    def mutate(pages):
        # page 2 claims itself as the next page — a cycle, never terminates
        pages["cur-0002"] = dict(pages["cur-0002"], next_page_token="cur-0002")
...
def test_truncated_page_is_detected():
    # page-001 claims issue_count=5 but carries only 4 rows — refused as
    # a truncated page, never silently accepted as 4 issues.
    with pytest.raises(ValueError, match="truncated page"):
        collect(transport=transport_for(TRUNCATED_DIR))


def test_all_issues_are_collected():
    records = collect().records
    assert len(records) == 7
    assert sum(1 for r in records if r.status == "Deployed") == 3
    assert sum(1 for r in records if r.status == "Approved") == 3
    assert sum(1 for r in records if r.status == "Rejected") == 1


def test_null_reporter_rows_are_surfaced_not_dropped():
    records = collect().records
    no_reporter = [r for r in records if r.key == "CHG-103"]
    assert len(no_reporter) == 1
    # a null reporter_email survives collection; the reconciler buckets
    # this identity UNRESOLVABLE and the verdict path ends
    # UNKNOWN_POPULATION (D-7/D-U1), never a silently dropped row
    assert no_reporter[0].reporter_email is None
