...
plus E117, E302, and the zero-collectors-on-error rule."""
...
def test_six_month_timing_vs_okta_90d_fails_with_e204_and_suggestion():
    ...
    report = compile_claims(okta_claims, okta_pop, REGISTRY)
...
def test_unregistered_system_fails_with_e117_naming_the_gap():
    ...
    report = compile_claims(tuple(instance.claims), tuple(instance.populations), REGISTRY)
...
def test_clean_state_claim_compiles_to_a_plan():
    ...
    report = compile_claims(non_timing, github_pop, ratified_workday_registry())
...
def test_e302_on_schema_version_drift():
    ...
    report = compile_claims((non_timing,), (github_pop,), REGISTRY, contracts=(drifted,))
