...
from aegis_sentinel.collectors.okta_users import (
...
def collect(transport=None, tenant=TENANT, period=PERIOD):
    return collect_okta_users(
        transport or transport_for(TENANT_DIR),
        tenant=tenant,
        period=period,
        population_ref="pop-okta-directory-users",
    )
...
def test_all_users_are_collected():
    ...
    assert sum(1 for r in records if r.status == "ACTIVE") == 3
    assert sum(1 for r in records if r.status == "DEPROVISIONED") == 1
    assert sum(1 for r in records if r.status == "SUSPENDED") == 1


def test_null_email_row_is_surfaced_not_dropped():
    records = collect().records
    deprovisioned = [r for r in records if r.status == "DEPROVISIONED"]
    assert len(deprovisioned) == 1
    # a null email survives collection; the reconciler buckets this
    # identity UNRESOLVABLE and the verdict path ends UNKNOWN_POPULATION
    # (D-7/D-U1), never a silently dropped row
    assert deprovisioned[0].email is None
    assert deprovisioned[0].login == "cleo.marsh@example.com"
