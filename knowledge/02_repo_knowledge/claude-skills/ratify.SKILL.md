...
Human-ratified is not human-originated: you (the agent) may draft any judgment artifact, but it takes effect only when Mike ratifies its hash. This skill prepares the ratification; Mike performs step 4 himself. If you find yourself executing step 4, stop — that is the exact leak the architecture exists to prevent.
...
4. **Mike ratifies** — he runs the insert as the human path (not the app role):
