...
scripts/
  ...
  auto-merge.sh               the merge arm's engine body (the fleet v2 body, byte-identical below the sentinel to a private sibling's; docs/MERGE-POLICY.md pins its sha) under this repo's janus:merge-config block: claude/ and fix/ heads, merge commits, linked issue optional
